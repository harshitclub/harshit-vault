---
title: "Cognitive Neuroscience Master Guide"
subtitle: "Focus, Memory, Physical Health, and High-Leverage Execution"
author: "Dr. Sahar Yousef (UC Berkeley)"
tags:
  - neuroscience
  - engineering
  - mathematics
...
```mermaid
flowchart TD
    DMN["Story Studios (DMN)\nIdentity, Narrative, Spontaneous Synthesis"]
    CEN["City Hall (CEN)\nGoal Execution, Analytical Logic, Suppression"]
    DAN["The Watchtower (DAN)\nConscious Focus Spotlight"]
    SN["Dispatch (Salience Network)\nInterrupt & Novelty Detector"]
    Amygdala["Alarm Tower (Amygdala)\nThreat & Urgency Detector"]
    Hippo["Archive (Hippocampus)\nLong-Term Memory Storage"]

    DMN -->|Supplies Vision & Direction| CEN
    CEN -->|Directs Focus| DAN
    SN -.->|Forces Interruptions| DAN
    Amygdala -.->|Overrides Analytical Focus| DAN
    DAN -->|Only Input Funnel| Hippo
```
...
```mermaid
flowchart LR
    A["1. Core Beliefs (DMN)"] --> B["2. Attentional Filter"] 
    B --> C["3. Daily Execution"] 
    C --> D["4. Real-World Proof"] 
    D -->|Reinforces| A
```
...
```mermaid
gantt
    title Monotasking vs. Fragmented Context Switching
    dateFormat X
    axisFormat %s

    section Monotask
    Deep Engineering Architecture (90 min uninterrupted): active, m1, 0, 90

    section Fragmented Work
    Code Logic (15 min)             :crit, f1, 0, 15
    Context Switch (5 min)          :f2, 15, 20
    Check Messages (10 min)         :crit, f3, 20, 30
    Rebuild Mental Cache (15 min)   :f4, 30, 45
    Code Logic (15 min)             :crit, f5, 45, 60
    Useful Work Lost (>50%)         :crit, f6, 60, 90
```
...
```mermaid
flowchart LR
    P1["1. Load Problem\n(Read Docs, Trace Stack, Map Math)"]
    --> P2["2. Disengage\n(Walk, Warm Shower, Prepare Food)"]
